...
from termhub.update import ncbi_preprocess
...
def getdeps(dpath, force=False, rd_fail="ask"):
    ...
    if not exists(dpath):
        mkdir(dpath)
        # New location -- any previous log file should be ignored.
    ...
    preproc_jobs = {}
    preprocessors = {}
    preprocessors["ncbi"] = {"module": ncbi_preprocess,
                             "args": (dpath+"gene_info",
                                      dpath+"gene_info.trunc",
                                      [1, 2, 4])}
    ...
    for dfile in dependencies:
        ...
        if not force and res_dfile in dependencies_log_dict:
            print("%-35s\tChecking for a newer version ... " % res_dfile, end=' ')
        else:
            print("%-35s\tFetching timestamp..." % res_dfile, end=' ')
